// @file: apps/web/e2e/instructor-application/instructor-application-smoke.e2e.spec.ts
...
async function ensureDraftApplication(
  ...
  await loginByKeycloakForApplications(page);
...
test.describe("instructor application smoke", () => {
  test.skip(
    !hasProfileCredentials("readonly"),
    "Requires HSS_E2E_READONLY_USERNAME and HSS_E2E_READONLY_PASSWORD.",
  );

  test("should keep the selected detail tab in the URL after reload", async ({
    page,
  }) => {
    const application = await ensureDraftApplication(page, 0);

    await page.goto(`${application.detailPath}?tab=attachments`, {
      waitUntil: "networkidle",
    });

    const attachmentsTab = page.getByRole("tab", {
      name: /^Załączniki i podsumowanie$|^Attachments & Summary$/i,
    });
    await expect(attachmentsTab).toHaveAttribute("aria-selected", "true");
    await expect(page).toHaveURL(
      new RegExp(
        `/${env.locale}/applications/${application.id}\\?tab=attachments$`,
        "i",
      ),
    );
  ...
  test("should keep the selected edit step in the URL after reload", async ({
    page,
  }) => {
    const application = await ensureDraftApplication(page, 0);

    await page.goto(`${application.editPath}?step=serviceHistory`, {
      waitUntil: "networkidle",
    });

    const serviceHistoryStep = page.getByRole("button", {
      name: /^Historia służby$|^Service History$/i,
    });

    await expect(serviceHistoryStep).toHaveAttribute("aria-current", "step");
    await expect(page).toHaveURL(
      new RegExp(
        `/${env.locale}/applications/${application.id}/edit\\?step=serviceHistory$`,
        "i",
      ),
    );
    await expect(
      page.getByRole("button", { name: /^Wstecz$|^Previous$/i }),
    ).toBeVisible();
