// @file: apps/web/src/app/[locale]/commission/layout.tsx
...
export default async function CommissionLayout({ children, params }: Props) {
  const { locale } = await params;
  const t = await getTranslations("common");
  const signInHref = buildKeycloakSignInHref(locale, `/${locale}/commission`);
  ...
  if (resolvedPrincipal.status === "unavailable") {
    return (
      <main className="mx-auto max-w-3xl px-6 py-10">
        <h1 className="text-2xl font-semibold">{t("nav.commission")}</h1>
        <AccessDenied
          code="503"
  ...
  if (!canAccess(resolvedPrincipal.principal, ROLE_RANK.COMMISSION_MEMBER)) {
    return (
      <main className="mx-auto max-w-3xl px-6 py-10">
        <h1 className="text-2xl font-semibold">{t("nav.commission")}</h1>
        <AccessDenied
